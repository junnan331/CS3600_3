...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        # if noisyDistance is None:  # If the ghost is captured
        #     allPossible[self.getJailPosition()] = 0
        # else:
        #     for p in self.allPositions:
        #         emissionModel = self.getObservationProb(noisyDistance, gameState.getPacmanPosition(), p, self.getJailPosition())
        #         trueDistance = util.manhattanDistance(p, gameState.getPacmanPosition())
        #         if p in self.legalPositions:
        #             allPossible[p] = emissionModel * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        l_count = len(self.legalPositions)
        par_position = self.numParticles // l_count

        for p in self.legalPositions:
            self.particles.extend([p] * par_position)

        # Distribute the remaining particles
        rem_pars = self.numParticles - (par_position * l_count)
        for i in range(rem_pars):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            observation_probability = self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
            temp[particle] += observation_probability
    ...
    def elapseTime(self, gameState):
        ...
        for oldP in self.particles:
            particle = self.getPositionDistribution(gameState, oldP).sample()
            newP.append(particle)
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            bd[particle] += 1  # Count each occurrence of a particle

        bd.normalize()  # Normalize the distribution
        return bd
